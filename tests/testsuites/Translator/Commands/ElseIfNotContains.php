    ...
    public function test_translateCommand()
    {
        ...
        $tests = array(
            array(
                ...
                'mailcode' => Mailcode_Factory::elseIfNotContains('FOO.BAR', 'Value'),
                'expected' => '#elseif(!$FOO.BAR.matches("(?s)Value"))'
            ),
            array(
                'label' => 'ElseIf not contains with slash',
                'mailcode' => Mailcode_Factory::elseIfNotContains('FOO.BAR', 'Va\lue'),
                'expected' => sprintf(
            ...
            array(
                ...
                'mailcode' => Mailcode_Factory::elseIfNotContains('FOO.BAR', '6 + 4 * 3'),
                'expected' => sprintf(
                    '#elseif(!$FOO.BAR.matches("(?s)%s"))',
                    '6 [SLASH]+ 4 [SLASH]* 3'
                )
            ...
            array(
                ...
                'mailcode' => Mailcode_Factory::elseIfNotContainsAny('FOO.BAR', array('Foo', 'Bar')),
                'expected' => '#elseif(!$FOO.BAR.matches("(?s)Foo") && !$FOO.BAR.matches("(?s)Bar"))'
            ),
            array(
                'label' => 'With quotes in search term',
                'mailcode' => Mailcode_Factory::elseIfNotContains('FOO.BAR', 'Value, "weird" huh?'),
                'expected' => sprintf(
                    '#elseif(!$FOO.BAR.matches("(?s)%s"))',
