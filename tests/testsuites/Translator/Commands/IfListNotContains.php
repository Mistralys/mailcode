...
final class Translator_Velocity_IfListNotContainsTests extends VelocityTestCase
{
    public function test_translateCommand()
    {
        ...
        $tests = array(
            array(
                ...
                'mailcode' => Mailcode_Factory::if()->listNotContains('FOO.BAR', array('Value')),
                'expected' => '#if(!$map.hasElement($FOO.list(), "BAR", "(?s)Value"))'
            ),
            array(
                'label' => 'If contains with slash',
                'mailcode' => Mailcode_Factory::if()->listNotContains('FOO.BAR', array('Va\lue')),
                'expected' => sprintf(
            ...
            array(
                ...
                'mailcode' => Mailcode_Factory::if()->listNotContains('FOO.BAR', array('6 + 4 * 3')),
                'expected' => sprintf(
                    '#if(!$map.hasElement($FOO.list(), "BAR", "(?s)%s"))',
                    '6 [SLASH]+ 4 [SLASH]* 3'
                )
            ...
            array(
                ...
                'mailcode' => Mailcode_Factory::if()->listNotContains('FOO.BAR', array('Foo', 'Bar')),
                'expected' => '#if(!$map.hasElement($FOO.list(), "BAR", "(?s)Foo") && !$map.hasElement($FOO.list(), "BAR", "(?s)Bar"))'
            ),
            array(
                'label' => 'With quotes in search term',
                'mailcode' => Mailcode_Factory::if()->listNotContains('FOO.BAR', array('Value, "weird" huh?')),
                'expected' => sprintf(
                    '#if(!$map.hasElement($FOO.list(), "BAR", "(?s)%s"))',
